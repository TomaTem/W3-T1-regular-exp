...
function hasSNILS(string) {
    let regExp = /\D\d{3}-\d{3}-\d{3}\s\d{2}(\D|$)/;
...
function formatSNILS(string) {
    let regexp = /(\D)(\d{3})\D*?(\d{3})\D*?(\d{3})\D*?(\d{2})(\D|$)/g
    return string.replace(regexp, '$1$2-$3-$4 $5$6')
}
